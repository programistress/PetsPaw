...
function SearchElement() {
  ...
  const chooseBreed = (id, name) => {
    setInput(name)
    setVisible(false)
    fetch(`https://api.thecatapi.com/v1/images/search?limit=67&breed_ids=${id}`, {
      headers: {
        'x-api-key': 'live_IvsY9IWZY2HevmQgUSlWy0ewC7J8szAIw2I0NWsSBkZ0TWuRyspIvq92umxAziyE',
      }
  ...
  return (
    ...
    <input className='search__input' placeholder='Search for breeds by name' value={input}
        onChange={(e) => handleChange(e.target.value)}/>
    <button className='search__btn'>
    </button>
    <div className={visible ? 'search__results' : 'search__results-hidden'}>
      {results.map((cat) => (
      <Link to='/search'  className='search__result' key={cat.id} onClick={() => chooseBreed(cat.id, cat.name)}>{cat.name}</Link>
      ))}
      </div>
    </div>
